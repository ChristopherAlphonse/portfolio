...
const Project = ({ item }) => {
  return (
    <div key={item.id} className="flex flex-col items-center text-center transform transition duration-700 hover:scale-110">
      <div className="mb-8">
        <img className="rounded-3xl w-[25rem]" src={item.image} alt="project " />
      </div>
      ...
      <ul class="inline-block list-none text-sm text-white font-semibold">
        <li class="inline-block bg-blue-400/50 px-3 rounded-full">Tailwind</li>
        <li class="inline-block bg-green-400/50 px-3 mx-2 rounded-full">
          React
        </li>
        <li class="inline-block bg-pink-400/50 px-3 rounded-full">
          TypeScript
        </li>
      </ul>
      <div>
